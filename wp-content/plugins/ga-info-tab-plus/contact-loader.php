...
class CONTACT extends BP_Group_Extension {
	...
	var $display_hook = 'groups_infotab_group_boxes';
	...
	function CONTACT(){
		...
		//$bp->groups->current_group->infotab = groups_get_groupmeta($bp->groups->current_group->id, 'info');
		
		// Display or Hide top menu from group non-members
		$this->visibility = /*$bp->groups->current_group->infotab['display_page'] ? $bp->groups->current_group->infotab['display_page'] :*/ 'public';
		$this->enable_nav_item = /*$bp->groups->current_group->infotab['display_page'] == 'public' ?*/ true /*: false*/;
                $this->enable_edit_item = current_user_can('manage_options');
                
		// In Admin
		$this->name = info_tab_plus_names('nav');
		// Public page
		$this->nav_item_name = info_tab_plus_names('name');
		...
		foreach( $fields as $field ){
			...
			switch($field->type){
				...
				case 'radio':
					...
					echo '</span>';
					if ($req) 
						echo '<a class="clear-value" href="javascript:clear( \'contact-' . $field->slug . '\' );">'. __( 'Clear', 'info' ) .'</a>';
					break;
				case 'datebox':
	...
	function edit_group_fields_save($group_id){
        ...
        function save_activity(){
            ...
            'action' => 'info_tab_plus_update', // BP bug. 'action' is type
            ...
            if ( !empty( $duplicate_activity['activities'] ) ) {
                ...
                if ( time() - $drunix <= apply_filters( 'bp_info_tab_plus_edit_activity_throttle_time', 60*45 ) )
                    return;
            }
            ...
            $args = array(
                'user_id'               => $bp->loggedin_user->id,
                'action'                => apply_filters( 'groups_activity_info_tab_plus_update_action',       $activity_action ),
                'primary_link'          => apply_filters( 'groups_activity_info_tab_plus_update_primary_link', $primary_link ),
                'component'		=> $component,
                'type'			=> 'info_tab_plus_update',
                ...
                'hide_sitewide'		=> apply_filters( 'bp_ga_info_tab_plus_hide_sitewide', false, $bp->groups->current_group->id, $component ) // Filtered to allow plugins and integration pieces to dictate
            );
            bp_activity_add( apply_filters( 'bp_info_tab_plus_activity_args', $args ) );
        }        
	...
	function edit_screen_general($bp){
		...
		if (empty($fields))
			$this->add_default_fields();		
		$public_checked = $bp->groups->current_group->infotab['display_page'] == 'public' ? 'checked="checked"' : '';
		$private_checked = $bp->groups->current_group->infotab['display_page'] == 'private' ? 'checked="checked"' : '';

		$this->edit_screen_head('general');
		
		echo '<p>';
			echo '<label for="group_infotab_display">'.sprintf(__('Do you want to make <strong>"%s"</strong> page public (extra group information will be displayed on this page)?','info'), $this->nav_item_name).'</label>';
			echo '<input type="radio" value="public" '.$public_checked.' name="group-infotab-display"> '.__('Show it', 'info').'<br />';
			echo '<input type="radio" value="private" '.$private_checked.' name="group-infotab-display"> '. __('Hide it', 'info');
		echo '</p>';
		...
		echo '<p>';
			$fields = $this->get_all_fields($bp->groups->current_group->id);
			if (!empty($fields)){
				echo '<label for="group_extras_display">'.sprintf(__('Please choose below the fields you want to show on <strong>"%s"</strong> page:','info'), $this->nav_item_name).'</label>';
		...
		echo '<p><input type="submit" name="save_general" id="save" value="'.__('Save Changes &rarr;','info').'"></p>';
		wp_nonce_field('groups_edit_group_infotab');
	}
			...
			foreach($fields as $field){
				echo '<li id="position_'.str_replace('_', '', $field->slug).'" class="default">
								...
								<span class="field-link">
									<a href="' . bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug . '/fields-manage/?edit=' . $field->slug . '" class="button" title="'.__('Change its title, description etc','info').'">'.__('Edit field', 'info').'</a>&nbsp;
									<a href="#" class="button delete_field" title="'.__('Delete this field and all its content', 'info').'">'.__('Delete', 'info').'</a>
								</span>
							</li>';
			}
		echo '</ul>';
	...
	function edit_screen_fields_manage($bp){
		...
		echo '<p>';
			echo '<label>' . __('Field Title', 'info') . '</label>';
			echo '<input type="text" value="'.$field->title.'" name="extra-field-title">';
			
			if (empty($field)){
				echo '<label>' . __('Field Type', 'info') . '</label>';
				echo '<select name="extra-field-type" id="extra-field-type">';
					echo '<option value="text">' . __('Text Box', 'info') . '</option>';
					echo '<option value="textarea">' . __('Multi-line Text Box', 'info') . '</option>';
					echo '<option value="checkbox">' . __('Checkboxes', 'info') . '</option>';
					echo '<option value="radio">' . __('Radio Buttons', 'info') . '</option>';
					//echo '<option value="datebox">' . __('Date Selector', 'info') . '</option>';
					echo '<option value="select">' . __('Drop Down Select Box', 'info') . '</option>';
				echo '</select>';
				
				echo '<div id="extra-field-vars">';
					echo '<div class="content"></div>';
					echo '<div class="links">
									<a class="button" href="#" id="add_new">' . __('Add New', 'info') . '</a>
							</div>';
				echo '</div>';
			}
			echo '<label>' . __('Field Description', 'info') . '</label>';
				echo '<textarea name="extra-field-desc">'.$field->title.'</textarea>';
			
			echo '<label for="extra-field-required">' . __('Is this field required (will be displayed on appropriate group creation step)?','info') . '</label>';
				$req = '';
				...
				echo '<input type="radio" value="1" '.$req.' name="extra-field-required"> '.__('Required', 'info').'<br />';
				echo '<input type="radio" value="0" '.$not_req.' name="extra-field-required"> '. __('Not Required', 'info');
				
			echo '<label for="extra-field-display">' . sprintf(__('Should this field be displayed for public on "<u>%s</u>" page?','info'), $this->nav_item_name) . '</label>';
				...
				echo '<input type="radio" value="1" '.$disp.' name="extra-field-display"> '.__('Display it', 'info').'<br />';
				echo '<input type="radio" value="0" '.$not_disp.' name="extra-field-display"> '. __('Do NOT display it', 'info');
		echo '</p>';
		
		if (empty($field)){
			echo '<p><input type="submit" name="save_fields_add" id="save" value="'.__('Create New &rarr;','info').'"></p>';
		}else{
			echo '<input type="hidden" name="extra-field-slug" value="' . $field->slug . '">';
			echo '<p><input type="submit" name="save_fields_edit" id="save" value="'.__('Save Changes &rarr;','info').'"></p>';
		}
		wp_nonce_field('groups_edit_group_infotab');
	}
	...
	function edit_screen_save() {
		global $bp;
		if ( $bp->current_component == $bp->groups->slug && 'info' == $bp->action_variables[0] ) {
			if ( !$bp->is_item_admin )
				return false;
			// Save general settings
			if ( isset($_POST['save_general'])){
				/* Check the nonce first. */
				if ( !check_admin_referer( 'groups_edit_group_infotab' ) )
					return false;
				
				$meta = array();
				$meta['display_page'] = $_POST['group-infotab-display'];
				
				// Save into groupmeta table
				groups_update_groupmeta( $bp->groups->current_group->id, 'info', $meta );
			...
			if ( isset($_POST['save_fields_add'])){
				...
				groups_update_groupmeta( $bp->groups->current_group->id, 'info_tab_plus_fields', $fields );
			...
			if ( isset($_POST['save_fields_edit'])){
				...
				groups_update_groupmeta( $bp->groups->current_group->id, 'info_tab_plus_fields', $updated );
	...
	function edit_screen_head($cur = 'general'){
		global $bp;
		if ($cur == 'general'){
			echo '<span class="extra-title">'.info_tab_plus_names('title_general').'</span>';
			echo '<span class="extra-subnav">
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug .'/'.'" class="button active">'. __('General', 'info') .'</a>
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug .'/fields/'.'" class="button">'. __('All Fields', 'info') .'</a>
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug .'/fields-manage/'.'" class="button">'. __('Add Fields', 'info') .'</a>
                                                <a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug .'/fields-reset/'.'" class="button">'. __('Restore Default Fields', 'info') .'</a>						
					</span>';
		}elseif ($cur == 'fields'){
			echo '<span class="extra-title">'.info_tab_plus_names('title_fields').'</span>';
			echo '<span class="extra-subnav">
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug .'/'.'" class="button">'. __('General', 'info') .'</a>
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug .'/fields/'.'" class="button active">'. __('All Fields', 'info') .'</a>
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug .'/fields-manage/'.'" class="button">'. __('Add Fields', 'info') .'</a>
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug .'/fields-reset/'.'" class="button">'. __('Restore Default Fields', 'info') .'</a>	
					</span>';
		}elseif ($cur == 'fields-reset'){
                        echo '<span class="extra-title">'.info_tab_plus_names('title_fields_reset').'</span>';
			echo '<span class="extra-subnav">
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug .'/'.'" class="button">'. __('General', 'info') .'</a>
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug .'/fields/'.'" class="button active">'. __('All Fields', 'info') .'</a>
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug .'/fields-manage/'.'" class="button">'. __('Add Fields', 'info') .'</a>
			...
			if ( isset($_GET['edit']) && !empty($_GET['edit']) ){
				echo '<span class="extra-title">'.info_tab_plus_names('title_fields_edit').'</span>';
				$active = '';
			}else{
				echo '<span class="extra-title">'.info_tab_plus_names('title_fields_add').'</span>';
				$active = 'active';
			}
			echo '<span class="extra-subnav">
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug . '/" class="button">'. __('General', 'info') .'</a>
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug . '/fields/" class="button">'. __('All Fields', 'info') .'</a>
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug . '/fields-manage/" class="button ' . $active . '">'. __('Add Fields', 'info') .'</a>
						<a href="'.bp_get_group_permalink( $bp->groups->current_group ) . 'admin/'.$this->slug .'/fields-reset/'.'" class="button">'. __('Restore Default Fields', 'info') .'</a>						
					</span>';
		}
	}
	...
	function get_all_fields($id){
		// get all fields
		$fields = array();
		$fields = groups_get_groupmeta($id, 'info_tab_plus_fields');
	...
	function remove_all_fields(){
		global $bp;		
		groups_delete_groupmeta($bp->groups->current_group->id, 'info_tab_plus_fields');
	}
	...
	function add_default_fields($group_id=NULL){
		...
		$fields = json_encode($fields);
		groups_update_groupmeta( $group_id, 'info_tab_plus_fields', $fields );		
	}	
	...
	function notices($type){
		switch($type){
			case 'updated';
				bp_core_add_message(__('Group Extras settings were succefully updated.','info'));
				break;
			case 'added';
				bp_core_add_message(__('New field was successfully added.','info'));
				break;
			case 'edted';
				bp_core_add_message(__('The field was successfully updated.','info'));
				break;
			case 'no-fields':
				echo '<div class="" id="message"><p>' . __('Please create at least 1 extra field.', 'info') . '</p></div>';
				break;
		}
	}
	...
	function ajax(){
		...
		switch($method){
			case 'reorder_fields':
				...
				groups_update_groupmeta( $bp->groups->current_group->id, 'info_tab_plus_fields', $new_order );
				die('saved');
				break;
			...
			case 'delete_field':
				...
				groups_update_groupmeta($bp->groups->current_group->id, 'info_tab_plus_fields', $left);
				die('deleted');
				break;
...
bp_register_group_extension('info');

add_action('wp_ajax_contact', 'info_tab_plus_ajax');
function info_tab_plus_ajax(){
