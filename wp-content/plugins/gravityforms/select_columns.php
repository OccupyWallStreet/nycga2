...
<html>
    <head>
        ...
        ?>
        <script src="<?php echo RGForms::get_base_url()?>/js/jquery-1.3.2.min.js"></script>
        <script src="<?php echo RGForms::get_base_url()?>/js/jquery-ui/ui.core.js"></script>
        <script src="<?php echo RGForms::get_base_url()?>/js/jquery-ui/ui.sortable.js"></script>
            ...
            <div class="gcolumn_container_left">
                ...
                <ul id="sortable_selected" class="sortable_connected">
                    ...
                    foreach($columns as $field_id => $field_info){
                        ?>
                        <li id="<?php echo $field_id?>"><?php echo RGForms::escape_text($field_info["label"]) ?></li>
                        <?php
                    }
                    ?>
                </ul>
                        ...
                        if(is_array($field["inputs"])){
                            foreach($field["inputs"] as $input){
                                if(!in_array($input["id"], $field_ids)){
                                    ?>
                                    <li id="<?php echo $input["id"]?>"><?php echo RGForms::escape_text(RGForms::get_label($field, $input["id"])) ?></li>
                                    <?php
                                }
                            }
                        }
                            ...
                            ?>
                            <li id="<?php echo $field["id"]?>"><?php echo  RGForms::escape_text($field["label"]) ?></li>
                            <?php
                        }
                    }
                    ?>
