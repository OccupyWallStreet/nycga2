<?php
/*
Plugin Name: GA Info Tab 
Plugin URI: http://nycga.net/
Description: Adding info extra group fields and some other missing functionality to groups
Version: 1.1
Author: Tech Ops
...
function contact_load(){
	global $bp;
	require ( dirname(__File__) . '/info-cssjs.php');
	if ( is_admin()){
		require ( dirname(__File__) . '/info-admin.php');
	}
	...
	if ( (is_string($contact['groups']) && $contact['groups'] == 'all' ) || (is_array($contact['groups']) && in_array($bp->groups->current_group->id, $contact['groups'])) ){
		require ( dirname(__File__) . '/info-loader.php');
	}
}
