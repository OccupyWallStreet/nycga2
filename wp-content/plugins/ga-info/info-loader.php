...
class CONTACT extends BP_Group_Extension {
			...
			foreach($fields as $field){
				...
				if ( $field->display != 1 || empty($data) || in_array($field->slug, $listed) ){
					...
					$emptytab=false;
								...
								// THIS IS WHERE TO CUSTOMIZE THE VALUES BEING DISPLAYED FORMATTINGs
				echo '<h4 title="' . ( ! empty($field->desc)  ? esc_attr($field->desc) : '')  .'">' . $field->title .'</h4>';
				
				$data = $this->auto_link($data);
				
				//begin matts playing around
				
				//$currentfieldtitle = $field->title;
				//$fieldtest = apply_filters('groups_ga_custom_tab',$data);
				
				//if($currentfieldtitle == "Facebook" && $data <> ""){
				//echo "<a href='http://www.facebook.com/$data' target='_blank'>$data $fieldtest</a>"; 
				//}
				//echo "$currentlabel $currentfieldtitle $data";
				//echo "currentfieldtitle: $currentfieldtitle";
				//echo "data:  $data";
	...
	function add_default_fields($group_id=NULL){
		...
		array_push($defaultfields, array('title'=>'Facebook','desc'=>'','type'=>'text','required'=>0,'display'=>1, 'slug'=>'facebook'));
		array_push($defaultfields, array('title'=>'External Website','desc'=>'','type'=>'text','required'=>0,'display'=>1, 'slug'=>'external_website'));
		array_push($defaultfields, array('title'=>'Other Contact Info','desc'=>'','type'=>'text','required'=>0,'display'=>1, 'slug'=>'other_contact'));
		array_push($defaultfields, array('title'=>'Charter-Detailed Description','desc'=>'','type'=>'text','required'=>0,'display'=>1, 'slug'=>'detailed_description'));
	...
	<input type="text" name="contact-facebook" id="contact-facebook" value="" ) />
    
	<label for="contact-external-website">External Website</label>
	<input type="text" name="contact-external_website" id="contact-external_website" value="" ) />
    
	<label for="contact-other-contact">Other Contact Info</label>
	<input type="text" name="contact-other_contact" id="contact-other_contact" value="" ) />
    
	<label for="contact-detailed-description">Detailed Description - Charter</label>
	<input type="text" name="contact-detailed_description" id="contact-detailed_description" value="" ) />
