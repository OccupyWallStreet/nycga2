...
if ( ! class_exists( 'WP_Facebook_Ai1ec' ) )
 ...
 * @since 1.0
 */
class Facebook_WP_Extend_Ai1ec extends WP_Facebook_Ai1ec {
	/**
  ...
  protected function setPersistentData($key, $value){
		...
		//WP 3.0+
		$this->fb_update_user_meta( get_current_user_id(), $key, $value);
	}
  ...
  protected function getPersistentData($key, $default = false){
    ...
    if (!in_array($key, self::$kSupportedKeys)) {
	  ...
	  return $usermeta = $this->fb_get_user_meta( get_current_user_id(), $key, true );
	}
  ...
  protected function clearPersistentData($key) {
    ...
    $this->fb_delete_user_meta( get_current_user_id(), $key);
  }
  ...
  /**
   *  Function added to the class to avoid incompatibilies with Facebook class
   */
  protected function fb_get_user_meta( $user_id, $meta_key, $single = false ) {
   ...
   *  Function added to the class to avoid incompatibilies with Facebook class
   */
  protected function fb_update_user_meta( $user_id, $meta_key, $meta_value, $prev_value = '' ) {
   ...
   *  Function added to the class to avoid incompatibilies with Facebook class
   */
  protected function fb_delete_user_meta( $user_id, $meta_key, $meta_value = '' ) {
