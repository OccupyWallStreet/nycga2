...
add_action( 'plugins_loaded', 'nycga_group_files_setup_globals', 5 );
	add_action( 'network_network_admin_menu', 'nycga_group_files_setup_globals', 2 );
...
function nycga_group_files_check_installed() {
	...
	add_action( 'network_network_admin_menu', 'nycga_group_files_check_installed',50);
...
function nycga_group_files_setup_nav() {
	...
	if( !get_option('nycga_group_files_enable_all_groups') && groups_get_groupmeta($bp->groups->current_group->id, 'group_files_documents_disabled'))
		return false;
...
function nycga_group_files_register_taxonomies() {
	register_taxonomy('group-files-category','group-file',array('hierarchical'=>true,'label'=>__('Group File Categories','nycga-group-files'),'query_var'=>false));
}
...
function NYCGA_GROUP_FILES_display_content() {
	...
	<?php if( $template->document_list && count($template->document_list >= 1) ) { ?>
		...
		<div class="pagination no-ajax">
			<div id="group-files-page-count" class="pag-count">
				<?php $template->pagination_count(); ?>
			</div>
		<?php if( $template->show_pagination() ){ ?>
			<div id="group-files-page-links" class="pagination-links">
		...
		foreach( $template->document_list as $document_params ) {
			...
			<a class="group-files-title" id="group-files-link-<?php echo $document->id; ?>" href="<?php $document->url(); ?>" target="_blank"><?php echo $document->name; ?>

			<?php if( get_option( 'nycga_group_files_display_file_size' )) { echo ' <span class="group-files-filesize">(' . get_file_size( $document ) . ')</span>'; } ?></a> &nbsp;
			
			<span class="group-files-meta"><?php printf( __( 'Uploaded by %s on %s', 'nycga-group-files'),bp_core_get_userlink($document->user_id),date( get_option('date_format'), $document->created_ts )); ?></span>

			<?php if( NYCGA_GROUP_FILES_SHOW_DESCRIPTIONS && $document->description ){ echo '<br /><span class="group-files-description">' . nl2br($document->description) . '</span>'; }

			//show edit and delete options if user is privileged
			echo '<div class="admin-links">';
			if( $document->current_user_can('edit') ) {
				$edit_link = wp_nonce_url( $template->action_link . 'edit/' . $document->id, 'group-files-edit-link' );
				echo "<a href='$edit_link'>" . __('Edit','nycga-group-files') . "</a> | ";
			}
			if( $document->current_user_can('delete') ) {
				$delete_link = wp_nonce_url( $template->action_link . 'delete/' . $document->id, 'group-files-delete-link' );
...
function nycga_group_files_delete( $id ) {
	...
	//check nonce
	if (!wp_verify_nonce($_REQUEST['_wpnonce'], 'group-files-delete-link')) {
