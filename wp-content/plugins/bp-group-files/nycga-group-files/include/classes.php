...
class NYCGA_Group_Files {
	...
	private function populate() {
		...
		if ( $row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$bp->group_files->table_name} WHERE id = %d", $this->id ) ) ) {
	 ...
	 */
	 public function populate_by_file( $file ) {
		...
		if ( $row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$bp->group_files->table_name} WHERE file LIKE '%s'", $file ) ) ) {
			foreach( $this as $field => $value ) {
				$this->$field = $row->$field;
			}
			return true;
	...
	public function save($check_file_upload = true) {
		...
		do_action( 'nycga_group_files_data_before_save', $this );

		if ( $this->id ) {
			// Update
			$result = $wpdb->query( $wpdb->prepare( 
					"UPDATE {$bp->group_files->table_name} SET 
						modified_ts = %d,
						name = %s,
						description = %s,
						featured = %d
			...
			$result = $wpdb->query( $wpdb->prepare( 
				"INSERT INTO {$bp->group_files->table_name} ( 
					user_id,
					group_id, 
					created_ts,
					modified_ts,
		...
		do_action( 'nycga_group_files_data_after_save', $this ); 
	...
	public function increment_download_count() {
		...
		return $wpdb->query( $wpdb->prepare( "UPDATE {$bp->group_files->table_name} SET download_count = (download_count + 1) WHERE id = %d", $this->id ) );
	}
	...
	public function delete() {
		...
		if( $this->current_user_can('delete') ) {
			do_action('nycga_group_files_data_before_delete', $this);
			if( $this->file && file_exists( $this->get_path(1) ) )
				@unlink( $this->get_path(1) );

			return $wpdb->query( $wpdb->prepare( "DELETE FROM {$bp->group_files->table_name} WHERE id = %d", $this->id ) );
		}
	}
	...
	private function UploadFile() {
		...
		if( !$_FILES['nycga_group_files_file']['name'] ) {
			bp_core_add_message( __('Whoops!  There was no file selected for upload.','nycga-group-files'),'error' );
			return false;
		}
		//check that file has an allowed extension
		if( !nycga_group_files_check_ext( $_FILES['nycga_group_files_file']['name'] ) ) {
			bp_core_add_message( __('The type of document submitted is not allowed','nycga-group-files'),'error' );	
			return false;
		}

		//if there was any upload errors, spit them out
		if( $_FILES['nycga_group_files_file']['error'] ) {
			switch( $_FILES['nycga_group_files_file']['error'] ) {
				case UPLOAD_ERR_INI_SIZE:
					bp_core_add_message( __('There was a problem; your file is larger than is allowed by the site administrator.','nycga-group-files'),'error');
				break;
				case UPLOAD_ERR_PARTIAL:
					bp_core_add_message( __('There was a problem; the file was only partially uploaded.','nycga-group-files'), 'error');
				break;
				case UPLOAD_ERR_NO_FILE:
					bp_core_add_message( __('There was a problem; no file was found for the upload.','nycga-group-files'),'error');
				break;
				case UPLOAD_ERR_NO_TMP_DIR:
					bp_core_add_message( __('There was a problem; the temporary folder for the file is missing.','nycga-group-files'),'error');
				break;
				case UPLOAD_ERR_CANT_WRITE:
					bp_core_add_message( __('There was a problem; the file could not be saved.','nycga-group-files'),'error');
				break;
			}
			return false;
		}
		...
		if ( !$this->name )
			if( get_magic_quotes_gpc() ){
				$this->name = stripslashes( basename( $_FILES['nycga_group_files_file']['name'] ) );
			} else {
				$this->name = basename( $_FILES['nycga_group_files_file']['name'] );
			}

		$this->file = apply_filters('nycga_group_files_filename_in',basename($_FILES['nycga_group_files_file']['name']));

		$file_path = $this->get_path(0,1);

		if( move_uploaded_file( $_FILES['nycga_group_files_file']['tmp_name'], $file_path ) ) {
			return true;
		} else {
			bp_core_add_message( __('There was a problem saving your file, please try again.','nycga-group-files'),'error');
			return false;
		}
	}
	...
	public function current_user_can( $action ) {
		...
		switch( $action ) {
			case 'add':
				switch( get_option( 'nycga_group_files_upload_permission' ) ) {
					case 'mods_decide':
						switch( groups_get_groupmeta( $bp->groups->current_group->id, 'group_files_upload_permission')) {
							case 'mods_only':
	...
	public function get_url($legacy_check = 1){
		...
		if( function_exists('bp_core_avatar_upload_path')) {
			$document_url = str_replace( WP_CONTENT_DIR, WP_CONTENT_URL, bp_core_avatar_upload_path() ) . '/group-files/' . $this->group_id . '/' . $this->file;
			...
			$document_url .= '/group-files/' . $this->group_id . '/' . $this->file;
		}
		...
		if( $legacy_check ) {
			...
			if( !file_exists( $document_path ) ) {
				...
				if( defined( 'NYCGA_GROUP_FILES_FILE_URL' ) ) {
					$document_url = NYCGA_GROUP_FILES_FILE_URL . $this->file;

				} else { //if not there, check legacy location
					$document_url = WP_PLUGIN_URL . '/nycga-group-files/documents/' . $this->file;
				}
			}
		}
		return apply_filters( 'nycga_group_files_file_url', $document_url, $this->group_id, $this->file );
	}
	...
	public function get_path( $legacy_check = 0, $create_folders = 0 ) {
		...
		/***
		 * place 'group-files' on the same level as 'group-avatars'
		 * organize docs within group sub-folders
		 */
		$document_dir = bp_core_avatar_upload_path() . '/group-files/' . $this->group_id;
		...
		if ( $legacy_check && !file_exists( $document_path ) ) {
			...
			/* check legacy override */
			if ( defined( 'NYCGA_GROUP_FILES_FILE_PATH' ) )
				$document_path = NYCGA_GROUP_FILES_FILE_PATH . $this->file;

			/* if not there, check legacy default */
			else
				$document_path = WP_PLUGIN_DIR . '/nyca-group-files/documents/' . $this->file;
		}

		return apply_filters( 'nycga_group_files_file_path', $document_path, $this->group_id, $this->file );
	}
	...
	public function icon() {
		...
		echo '<a class="group-files-icon" id="group-document-icon-' . $this->id . '" href="' . $this->get_url() . '" target="_blank"><img class="nycga-group-files-icon" src="' . $icon_url . '" alt="" /></a>';
		}
	}
		public function get_icon() {
			...
			$img_folder = WP_PLUGIN_URL . '/nycga-group-files/images/icons/';

			$img_url = $img_folder . $icons[$extension];

			return apply_filters('nycga_group_files_get_icon',$img_url, $this->group_id, $this->file);
		}
	...
	public static function get_ids_in_current_group() {
		...
		return $wpdb->get_col( $wpdb->prepare( "SELECT id FROM {$bp->group_files->table_name} WHERE group_id = %d", $group_id));
	}

	public static function get_total( $group_id, $category = false ) {
		global $wpdb, $bp;

		$sql = "SELECT COUNT(*) FROM {$bp->group_files->table_name} WHERE group_id = %d ";
		if( $category ) {
			//grab all object id's in the passed category
			$category_ids = get_objects_in_term($category,'group-files-category');
	...
	public static function get_list_by_group( $group_id, $category=0, $sort=0, $order=0, $start=0, $items=0 ){
		...
		if( !$category && !$sort ) {
			...
			$result = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$bp->group_files->table_name} WHERE group_id = %d ORDER BY name ASC", $group_id), ARRAY_A );
		...
		$sql = "SELECT * FROM {$bp->group_files->table_name} WHERE group_id = %d ";
		if( !empty( $category_ids ) ) {
	...
	public static function get_list_for_newest_widget($num, $group_filter=0, $featured=0 ) {
		...
		if( $group_filter || $featured ) {
			$sql = "SELECT * FROM {$bp->group_files->table_name} WHERE 1=1 ";
			if( $group_filter )
				$sql .= "AND group_id = $group_filter ";
			if( $featured && NYCGA_GROUP_FILES_FEATURED )
				$sql .= "AND featured = 1 ";
			$sql .= "ORDER BY created_ts DESC LIMIT %d";
			$result = $wpdb->get_results( $wpdb->prepare( $sql, $num), ARRAY_A );

		} else {
			$result = $wpdb->get_results( $wpdb->prepare( "SELECT d.* FROM {$bp->group_files->table_name} d INNER JOIN {$bp->groups->table_name} g ON d.group_id = g.id WHERE g.status = 'public' ORDER BY created_ts DESC LIMIT %d", $num), ARRAY_A );
		}
	...
	public static function get_list_for_popular_widget($num, $group_filter=0, $featured=0 ) {
		...
		if( $group_filter || $featured ) {
			...
			$result = $wpdb->get_results( $wpdb->prepare( "SELECT d.* FROM {$bp->group_files->table_name} d INNER JOIN {$bp->groups->table_name} g ON d.group_id = g.id WHERE g.status = 'public' ORDER BY download_count DESC LIMIT %d", $num), ARRAY_A );
		}
