...
function bp_group_documents_front_cssjs() {
	...
	if ( $bp->current_component == $bp->groups->slug ) {
		...
		wp_enqueue_script( 'bp-group-documents', WP_PLUGIN_URL . '/bp-group-files/js/general.js',array('jquery') );
		wp_enqueue_style('bp-group-documents',WP_PLUGIN_URL . '/bp-group-files/css/style.css');
		...
		if( $bp->current_action == 'forum' && get_option('bp_group_documents_forum_attachments') ) {
			wp_enqueue_script('bp-group-documents-forums', WP_PLUGIN_URL . '/bp-group-files/js/forum.js');
		}

		switch( BP_GROUP_DOCUMENTS_THEME_VERSION ){
			case '1.1':
				wp_enqueue_style('bp-group-documents-1.1', WP_PLUGIN_URL . '/bp-group-files/css/11.css');
			break;
			case '1.2':
			//	wp_enqueue_style('bp-group-documents-1.2', WP_PLUGIN_URL . '/bp-group-files/css/12.css');
			break;
		}
	}
}
...
function bp_group_documents_admin_cssjs() {
	wp_enqueue_style('bp-group-documents-admin',WP_PLUGIN_URL . '/bp-group-files/css/admin.css');
	wp_enqueue_script( 'bp-group-documents-admin', WP_PLUGIN_URL . '/bp-group-files/js/admin.js', array('jquery') );
}
