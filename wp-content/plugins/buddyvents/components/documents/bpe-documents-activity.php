...
/**
 * Add an activity entry when event documents get updated
 * 
 * Attached to the <code>bpe_updated_event_documents</code> action hook
 * 
 * @package Documents
 * @since 	1.7
 * 
 ...
 */
function bpe_add_updated_event_documents_activity( $event )
{
	...
	bpe_record_activity( array(	
		'id' 			=> $act_id,
		'action' 		=> apply_filters( 'bpe_activity_action_updated_document', sprintf( __( '%1$s has updated the documents for the event <a href="%2$s">%3$s</a>', 'events' ), bp_core_get_userlink( bpe_get_event_user_id( $event ) ), $event_link, bpe_get_event_name( $event ) ), $event_link, $event ),
...
add_action( 'bpe_updated_event_documents', 'bpe_add_updated_event_documents_activity' );
 ...
 * @uses 	bpe_get_option()
 */
function bpe_add_documents_activity_filter()
{
	$filter = '';

	if( in_array( bpe_get_option( 'enable_documents' ), array( 1, 2, 3 ) ) )
	    $filter .= '<option value="updated_event_document">'. __( 'Updated event document', 'events' ) .'</option>';

	return $filter;
}
add_action( 'bpe_activity_filters', 'bpe_add_documents_activity_filter' );
 ...
 * @uses 	bpe_get_option()
 */
function bpe_register_documents_activity_actions()
{
...
add_action( 'bp_register_activity_actions', 'bpe_register_documents_activity_actions' );
?>