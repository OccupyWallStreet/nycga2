...
?>
<h3 class="pagetitle">
	<?php printf( __( 'Month: %s %s', 'events' ), mysql2date( 'F', bp_action_variable( 1 ) .'-'. bp_action_variable( 0 ), true ), bp_action_variable( 1 ) ) ?><?php if( ! bpe_is_restricted() ) : ?>&nbsp;<a class="button" href="<?php echo bp_get_root_domain() . '/' . bpe_get_base( 'root_slug' ) . '/'. bpe_get_option( 'create_slug' ) ?>"><?php _e( 'Create Event', 'events' ) ?></a><?php endif; ?>
</h3>
...
<?php if( bpe_has_events() ) : ?>
    ...
    <?php do_action( 'bpe_month_before_loop' ) ?>
    ...
    <?php do_action( 'bpe_month_inside_loop' ) ?>
	
	</ul>

    <?php do_action( 'bpe_month_after_loop' ) ?>
