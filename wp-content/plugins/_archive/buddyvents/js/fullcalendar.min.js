...
jQuery(document).ready(function() {
	calendar = jQuery('#fullcalendar').fullCalendar({
		events: function(start, end, callback) {
			var head = jQuery('.fc-header-title h2');
			head.addClass('fcal-loading');
			...
			req = jQuery.post( ajaxurl, {
				...
				'_ajax_nonce': fullCal.eventNonce,
				'action': 'bpe_fullcalendar_get_calevent',
				'cookie': encodeURIComponent(document.cookie)
			},
			function(response) {
				head.removeClass('fcal-loading');
				...
				jQuery.each(response, function(key, value) {
					events.push({
						...
						desc: value.desc,
						allDay: ((value.allday == 'yes') ? true : false)
					});
				});
				req = null;
		...
		eventClick: function(calEvent, jsEvent, view) {
			var title = jQuery(jsEvent.currentTarget).find('.fc-event-title');
			title.addClass('fcal-loading');

			var startMonth = (calEvent.start.getMonth() < 9 ? '0' : '') + (calEvent.start.getMonth()+1);
			var startDay = (calEvent.start.getDate() < 10 ? '0' : '') + calEvent.start.getDate();
			var startYear = calEvent.start.getFullYear();

			var endMonth = (calEvent.end.getMonth() < 9 ? '0' : '') + (calEvent.end.getMonth()+1);
			var endDay = (calEvent.end.getDate() < 10 ? '0' : '') + calEvent.end.getDate();
			var endYear = calEvent.end.getFullYear();

			jQuery.post( ajaxurl, {
				'_ajax_nonce': fullCal.clickNonce,
				'action': 'bpe_fullcalendar_click_calevent',
				'event_id' : calEvent.id,
				'parent_id' : calEvent.parentId,
				'begin': startYear+'-'+startMonth+'-'+startDay,
				'end': endYear+'-'+endMonth+'-'+endDay,
				'cookie': encodeURIComponent(document.cookie)
			},
				...
				jQuery.colorbox({
					...
					onComplete:function(){
						if(parseInt(response.hasLocation) == 1 ) {
							var mapId = (parseInt(calEvent.parentId) == 0) ? calEvent.id : calEvent.parentId;
							eval( 'map_init' + mapId +'()' );
						}
					}
				});
				
				title.removeClass('fcal-loading');
			});
		},
		dayClick: function(date, allDay, jsEvent, view) {
			if(view.name == 'month'){
				var num = jQuery(jsEvent.currentTarget).find('.fc-day-number');
				num.addClass('fcal-loading');
				...
				jQuery.post( ajaxurl, {
					'_ajax_nonce': fullCal.dayNonce,
					'action': 'bpe_fullcalendar_day_click',
					'day' : d,
					'cookie': encodeURIComponent(document.cookie)
				},
				function(response) {
					if( response == '-1' ) {
						num.removeClass('fcal-loading');
						return false;
					}
					else if( response == '1' )
						window.location.href = fullCal.dayLink+d;
				});
			}
		},
		month: parseInt(fullCal.defMonth),
		year: parseInt(fullCal.defYear),
		eventColor: fullCal.eventColor,
		header: { left: fullCal.headLeft, center: fullCal.headCenter, right: fullCal.headRight },
		buttonText: { today: fullCal.today,	month: fullCal.month, week: fullCal.week, day: fullCal.day,	prev: fullCal.prev,	next: fullCal.next,	prevYear: fullCal.prevYear,	nextYear: fullCal.nextYear },
		columnFormat: {	month: fullCal.colMonth, week: fullCal.colWeek,	day: fullCal.colDay	},
		defaultView: fullCal.defView,
		monthNames: [fullCal.longJan, fullCal.longFeb, fullCal.longMar, fullCal.longApr, fullCal.longMay, fullCal.longJun, fullCal.longJul, fullCal.longAug, fullCal.longSep, fullCal.longOct, fullCal.longNov, fullCal.longDec],
		monthNamesShort: [fullCal.shortJan, fullCal.shortFeb, fullCal.shortMar, fullCal.shortApr, fullCal.shortMay, fullCal.shortJun, fullCal.shortJul, fullCal.shortAug, fullCal.shortSep, fullCal.shortOct, fullCal.shortNov, fullCal.shortDec],
		dayNames: [fullCal.longSun, fullCal.longMon, fullCal.longTue, fullCal.longWed, fullCal.longThu, fullCal.longFri, fullCal.longSat],
		dayNamesShort: [fullCal.shortSun, fullCal.shortMon, fullCal.shortTue, fullCal.shortWed, fullCal.shortThu, fullCal.shortFri, fullCal.shortSat],
		timeFormat: { agenda: fullCal.formatAgenda,	'': fullCal.devFormat },
		titleFormat: { month: fullCal.titleMonth, week: fullCal.titleWeek, day: fullCal.titleDay },
		firstDay: parseInt(fullCal.firstDay),
		axisFormat: fullCal.axisFormat,
		slotMinutes : parseInt(fullCal.minSlot),
		allDayText : fullCal.allDay,
		minTime: parseInt(fullCal.minTime),
		maxTime: parseInt(fullCal.maxTime)
	});
});