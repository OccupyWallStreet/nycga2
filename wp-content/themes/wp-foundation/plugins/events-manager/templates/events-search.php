 ...
 */
?>
<div class="em-events-search panel">
	<?php 
	...
	<form action="<?php echo EM_URI; ?>" method="post" class="em-events-search-form">
		...
		<span class="em-events-search-dates em-date-range">
			<input type="text" class="em-date-input-loc em-date-start" placeholder="Starting Date..." />
			<input type="hidden" class="em-date-input" name="scope[0]" value="<?php if( !empty($_REQUEST['scope'][0]) ) echo esc_attr($_REQUEST['scope'][0]); ?>" />
			<?php _e('and','dbem'); ?>
			<input type="text" class="em-date-input-loc em-date-end" placeholder="Ending Date..."/>
		...
		<input type="submit" value="<?php echo get_option('dbem_serach_form_submit','Search'); ?>" class="em-events-search-submit button" />		
	</form>	
</div>