...
function gallery_setup() {
	...
	add_action( 'wp_print_styles', 'gallery_enqueue_styles' );
	add_action( 'wp_head', 'gallery_themeoptions_output' );
	...
	$functcustomheader_on = get_option('dev_gallery_customheader_on');
	...
	require( dirname( __FILE__ ) . '/library/functions/custom-post-gallery.php' );
	require( dirname( __FILE__ ) . '/library/functions/test-data.php' );
...
if ( ! function_exists( 'gallery_enqueue_styles' ) ) :
function gallery_enqueue_styles(){
	global $themename, $shortname, $options, $options2, $options3, $bp_existed, $multi_site_on;
	$version = '2';
	
	if ($bp_existed){	
	wp_enqueue_style( 'gallery-buddypress', get_template_directory_uri() . '/_inc/css/gallerybuddypress.css', array(), $version );
	}	
	
	wp_enqueue_style( 'gallery', get_template_directory_uri() . '/_inc/css/gallery.css', array(), $version );
...
if ( ! function_exists( 'gallery_themeoptions_output' ) ) :
function gallery_themeoptions_output(){
include (get_template_directory() . '/library/options/options.php'); 
$get_current_scheme = get_option('dev_gallery_custom_style');

if (($get_current_scheme == "") || ($get_current_scheme == 'lightroom.css') || ($get_current_scheme == 'default.css')) { ?>
<link rel="stylesheet" href="<?php echo get_template_directory_uri(); ?>/library/styles/lightroom.css" type="text/css" media="all" />
<?php
}	else { ?>
...
<?php }
 ...
 print "</style>"; 
	$shadows = get_option('dev_gallery_shadows'); 
		if ($shadows == "off"){
	?>
		<link rel="stylesheet" href="<?php echo get_template_directory_uri(); ?>/_inc/css/noshadows.css" type="text/css" media="all" />
	<?php } ?>
	<link rel="stylesheet" href="<?php echo get_template_directory_uri(); ?>/library/scripts/nivo-slider/nivo-slider.css" type="text/css" media="all" />
	<link rel="stylesheet" href="<?php echo get_template_directory_uri(); ?>/library/scripts/fancybox/jquery.fancybox-1.3.4.css" type="text/css" media="all" />
...
register_sidebar(
	array(
		'name'          => __( 'Footer widget one', 'gallery' ),
		'id'            => 'sidebar-one',
...
register_sidebar(
	array(
		'name'          => __( 'Footer widget two', 'gallery' ),
		'id'            => 'sidebar-two',
...
register_sidebar(
	array(
		'name'          => __( 'Footer widget three', 'gallery' ),
		'id'            => 'sidebar-three',
...
register_sidebar(
	array(
		'name'          => __( 'Footer widget four', 'gallery' ),
		'id'            => 'sidebar-four',
		'description'   => 'Footer widget four',
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
        'after_widget' => '</div>',
        'before_title' => '<h3 class="widgettitle">',
        'after_title' => '</h3>'
...
register_sidebar(
	array(
		'name'          => __( 'BuddyPress sidebar', 'gallery' ),
		'id'            => 'sidebar-members',
...
function wpmudev_comment_form( $default_labels ) {
	...
	$fields =  array(
		'author' => '<p class="comment-form-author">' . '<label for="author">' . __( 'Name', 'gallery' ) . ( $req ? '<span class="required"> *</span>' : '' ) . '</label> ' .
		            '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
		'email'  => '<p class="comment-form-email"><label for="email">' . __( 'Email', 'gallery' ) . ( $req ? '<span class="required"> *</span>' : '' ) . '</label> ' .
		            '<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>',
		'url'    => '<p class="comment-form-url"><label for="url">' . __( 'Website', 'gallery' ) . '</label>' .
	...
	$new_labels = array(
		...
		'logged_in_as'   => '',
		'must_log_in'    => '<p class="alert">' . sprintf( __( 'You must be <a href="%1$s">logged in</a> to post a comment.', 'gallery' ), wp_login_url( get_permalink() ) )	. '</p>',
		'title_reply'    => __( 'Leave a reply', 'gallery' )
	);
	...
	<li <?php comment_class() ?> id="comment-<?php comment_ID() ?>">
		...
		<div class="comment-content">
			<div class="comment-meta">
				<p>
					<?php
						printf( __( '<a href="%1$s" rel="nofollow">%2$s</a> said on <a href="%3$s"><span class="time-since">%4$s</span></a>', 'gallery' ), get_comment_author_url(), get_comment_author(), get_comment_link(), get_comment_date() );
					?>
				</p>
			</div>
			...
			<div class="comment-entry">
				<?php if ( $comment->comment_approved == '0' ) : ?>
				 	<em class="moderate"><?php _e( 'Your comment is awaiting moderation.', 'gallery' ); ?></em>
				<?php endif; ?>
			...
			<div class="comment-options">
					...
					<?php if ( current_user_can( 'edit_comment', $comment->comment_ID ) ) : ?>
						<?php printf( '<a class="button comment-edit-link bp-secondary-action" href="%1$s" title="%2$s">%3$s</a> ', get_edit_comment_link( $comment->comment_ID ), esc_attr__( 'Edit comment', 'gallery' ), __( 'Edit', 'gallery' ) ) ?>
					<?php endif; ?>
	...
	<li <?php comment_class() ?> id="comment-<?php comment_ID() ?>">
		...
		<div class="comment-content">
			...
			<div class="comment-options">
					...
					<?php if ( current_user_can( 'edit_comment', $comment->comment_ID ) ) : ?>
						<?php printf( '<a class="button comment-edit-link" href="%1$s" title="%2$s">%3$s</a> ', get_edit_comment_link( $comment->comment_ID ), esc_attr__( 'Edit comment', 'gallery' ), __( 'Edit', 'gallery' ) ) ?>
					<?php endif; ?>
