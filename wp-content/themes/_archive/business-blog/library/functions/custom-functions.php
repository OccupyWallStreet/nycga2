...
register_nav_menus( array(
	'primary' => __( 'Primary Navigation', 'business-blog' ),
) );
...
$output = __('There is no excerpt because this is a protected post.', 'business-blog');
return $output;
}
}
...
function wpbreadcrumbs(){
	if (is_page() && !is_front_page() || is_single() || is_category()) {
	        ...
	        echo '<ul class="breadcrumbs">';
	        echo '<li class="front_page"><a href="'.get_bloginfo('url').'">'.get_bloginfo('name').'</a></li>';
	        echo '<li class="current">'.__( 'Home', 'business-blog' ).'</li>';
	        echo '</ul>';
	    }
}
...
function bpbreadcrumbs(){
	if (is_page() && !is_front_page() || is_single() || is_category()) {
		...
		if (bp_is_page( BP_ACTIVITY_SLUG )){
			 echo '<li>'.__( 'Activity', 'business-blog' ).'</li>';
		}
		if (bp_is_page( BP_MEMBERS_SLUG ) || bp_is_member()){
			 echo '<li class="current">'.__( 'Members', 'business-blog' ).'</li>';
		}
			if (bp_is_page( BP_GROUPS_SLUG ) || bp_is_group()){
				 echo '<li class="current">'.__( 'Groups', 'business-blog' ).'</li>';
			}
				if (bp_is_page( BP_FORUMS_SLUG )){
					 echo '<li class="current">'.__( 'Forums', 'business-blog' ).'</li>';
				}
						if (bp_is_page( BP_BLOGS_SLUG )){
							 echo '<li class="current">'.__( 'Blogs', 'business-blog' ).'</li>';
						}
						echo '</ul>';
...
function signup_button(){
	...
	$signupfeat_buttontext = get_option('dev_businessblog_signupfeat_buttontext');
	$signupfeat_buttontext_custom = get_option('dev_businessblog_signupfeat_buttontextcustom');
