<?php
	$bp_text = get_option('dev_bussinessblog_bp_text');
?>
		...
		<?php if ( is_user_logged_in() ) : ?>
			...
			<a class="button logout" href="<?php echo wp_logout_url( bp_get_root_domain() ) ?>"><?php _e( 'Log Out', 'business-blog') ?></a>
		...
		<?php else : ?>
						...
						<div class="label-box">	<h3>User Login:</h3>
							<span>
									<?php _e( 'To start connecting please log in first.', 'business-blog' ) ?>
									<?php if ( bp_get_signup_allowed() ) : ?>
										<?php printf( __( ' You can also <a href="%s" title="Create an account">create an account</a>.', 'business-blog' ), site_url( BP_REGISTER_SLUG . '/' ) ) ?>
									<?php endif; ?></span></div>
					<form name="loginform" id="logs" class="standard-form" action="<?php echo site_url( 'wp-login.php', 'login' ) ?>" method="post">

		<label for="username"><?php _e( 'Username', 'business-blog' ) ?></label>
		<input type="text" name="log" id="sidebar-user-login" class="input" value="<?php echo esc_attr(stripslashes($user_login)); ?>" tabindex="97" />

		<label for="username"><?php _e( 'Password', 'business-blog' ) ?></label>
