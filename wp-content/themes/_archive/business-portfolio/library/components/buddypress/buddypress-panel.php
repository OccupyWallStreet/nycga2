<?php
	$bp_text = get_option('dev_businessportfolio_bp_text');
?>
		<div id="memberpanel">
		<?php if ( is_user_logged_in() ) : ?>
			...
			<div id="sidebar-me">
				...
				<a class="button logout" href="<?php echo wp_logout_url( bp_get_root_domain() ) ?>"><?php _e( 'Log Out', 'business-portfolio') ?></a><br /><br />
		...
		<?php else : ?>
						...
						<div class="label-box">	<h3>User Login:</h3>
							<span>
									<?php _e( 'To start connecting please log in first.', 'business-portfolio' ) ?>
									<?php if ( bp_get_signup_allowed() ) : ?>
										<?php printf( __( ' You can also <a href="%s" title="Create an account">create an account</a>.', 'business-portfolio' ), site_url( BP_REGISTER_SLUG . '/' ) ) ?>
									<?php endif; ?></span></div>
		...
		<label for="username"><?php _e( 'Username', 'business-portfolio' ) ?></label>
			<input type="text" name="log" id="sidebar-user-login" class="input" value="<?php echo esc_attr(stripslashes($user_login)); ?>" tabindex="97" />

		<label for="username"><?php _e( 'Password', 'business-portfolio' ) ?></label>
