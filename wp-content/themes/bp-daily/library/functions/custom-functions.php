...
register_nav_menus( array(
	'primary' => __( 'Primary Navigation', 'bp-daily' ),
) );
...
$output = __('There is no excerpt because this is a protected post.', 'bp-daily');
return $output;
}
}
...
function wpbreadcrumbs(){
	...
	if (is_page() && !is_front_page() || is_single() || is_category()) {
	        ...
	        echo '<ul class="breadcrumbs">';
	        echo '<li class="front_page"><a href="'.get_bloginfo('url').'">'.get_bloginfo('name').'</a></li>';
	        echo '<li class="current">'.__( 'Home', 'bp-daily' ).'</li>';
	        echo '</ul>';
	    }
}
...
function bpbreadcrumbs(){
	if (is_page() && !is_front_page() || is_single() || is_category()) {
		...
		if (bp_is_page( BP_ACTIVITY_SLUG )){
			 echo '<li>'.__( 'Activity', 'bp-daily' ).'</li>';
		}
		if (bp_is_page( BP_MEMBERS_SLUG ) || bp_is_member()){
			 echo '<li class="current">'.__( 'Members', 'bp-daily' ).'</li>';
		}
			if (bp_is_page( BP_GROUPS_SLUG ) || bp_is_group()){
				 echo '<li class="current">'.__( 'Groups', 'bp-daily' ).'</li>';
			}
				if (bp_is_page( BP_FORUMS_SLUG )){
					 echo '<li class="current">'.__( 'Forums', 'bp-daily' ).'</li>';
				}
						if (bp_is_page( BP_BLOGS_SLUG )){
							 echo '<li class="current">'.__( 'Blogs', 'bp-daily' ).'</li>';
						}
						echo '</ul>';
...
function signup_button(){
	...
	$signupfeat_buttontext = get_option('dev_buddydaily_signupfeat_buttontext');
	$signupfeat_buttontext_custom = get_option('dev_buddydaily_signupfeat_buttontextcustom');
	
	if (($bp_existed == 'true') && ($signupfeat_buttontext_custom == "")){
	?>
		<a href="<?php echo home_url(); ?>/register/" class="button"><?php echo $signupfeat_buttontext; ?></a>
		...
		if ($multi_site_on == 'true'){
				?>
				<a href="<?php echo home_url(); ?>/wp-login.php?action=register" class="button"><?php echo $signupfeat_buttontext ?></a>
				<?php
		}
		else{
			?>
		  <a href="<?php echo home_url(); ?>/wp-login.php" class="button"><?php echo $signupfeat_buttontext ?></a>
		<?php
		}
	}
