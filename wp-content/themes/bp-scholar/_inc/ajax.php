...
function bp_dtheme_post_update() {
	...
	if ( empty( $_POST['content'] ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'Please enter some content to post.', 'bp-scholar' ) . '</p></div>';
		return false;
	}
	...
	if ( empty( $activity_id ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'There was a problem posting your update, please try again.', 'bp-scholar' ) . '</p></div>';
		return false;
	}
...
function bp_dtheme_new_activity_comment() {
	...
	if ( empty( $_POST['content'] ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'Please do not leave the comment area blank.', 'bp-scholar' ) . '</p></div>';
		return false;
	}

	if ( empty( $_POST['form_id'] ) || empty( $_POST['comment_id'] ) || !is_numeric( $_POST['form_id'] ) || !is_numeric( $_POST['comment_id'] ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'There was an error posting that reply, please try again.', 'bp-scholar' ) . '</p></div>';
		return false;
	}
...
function bp_dtheme_delete_activity() {
	...
	if ( !bp_activity_delete( array( 'id' => $activity->id, 'user_id' => $activity->user_id ) ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'There was a problem when deleting. Please try again.', 'bp-scholar' ) . '</p></div>';
		return false;
	}
...
function bp_dtheme_ajax_invite_user() {
	...
	if ( 'invite' == $_POST['friend_action'] ) {
		...
		echo '<div class="action">
				<a class="button remove" href="' . wp_nonce_url( $bp->loggedin_user->domain . bp_get_groups_slug() . '/' . $_POST['group_id'] . '/invites/remove/' . $user->id, 'groups_invite_uninvite_user' ) . '" id="uid-' . esc_attr( $user->id ) . '">' . __( 'Remove Invite', 'bp-scholar' ) . '</a>
			  </div>';
		echo '</li>';
...
function bp_dtheme_ajax_addremove_friend() {
	...
	if ( 'is_friend' == BP_Friends_Friendship::check_is_friend( $bp->loggedin_user->id, $_POST['fid'] ) ) {
		...
		if ( !friends_remove_friend( $bp->loggedin_user->id, $_POST['fid'] ) ) {
			echo __("Friendship could not be canceled.", 'bp-scholar');
		} else {
			echo '<a id="friend-' . $_POST['fid'] . '" class="add" rel="add" title="' . __( 'Add Friend', 'bp-scholar' ) . '" href="' . wp_nonce_url( $bp->loggedin_user->domain . bp_get_friends_slug() . '/add-friend/' . $_POST['fid'], 'friends_add_friend' ) . '">' . __( 'Add Friend', 'bp-scholar' ) . '</a>';
		}
		...
		if ( !friends_add_friend( $bp->loggedin_user->id, $_POST['fid'] ) ) {
			echo __("Friendship could not be requested.", 'bp-scholar');
		} else {
			echo '<a href="' . $bp->loggedin_user->domain . bp_get_friends_slug() . '/requests" class="requested">' . __( 'Friendship Requested', 'bp-scholar' ) . '</a>';
		}
	} else {
		echo __( 'Request Pending', 'bp-scholar' );
	}
...
function bp_dtheme_ajax_accept_friendship() {
	...
	if ( !friends_accept_friendship( $_POST['id'] ) )
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem accepting that request. Please try again.', 'bp-scholar' ) . '</p></div>';
...
function bp_dtheme_ajax_reject_friendship() {
	...
	if ( !friends_reject_friendship( $_POST['id'] ) )
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem rejecting that request. Please try again.', 'bp-scholar' ) . '</p></div>';
	...
	if ( !groups_is_user_member( $bp->loggedin_user->id, $group->id ) ) {
		...
		if ( 'public' == $group->status ) {
			...
			if ( !groups_join_group( $group->id ) ) {
				_e( 'Error joining group', 'bp-scholar' );
			} else {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="leave-group" rel="leave" title="' . __( 'Leave Group', 'bp-scholar' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'leave-group', 'groups_leave_group' ) . '">' . __( 'Leave Group', 'bp-scholar' ) . '</a>';
			}
			...
			if ( !groups_send_membership_request( $bp->loggedin_user->id, $group->id ) ) {
				_e( 'Error requesting membership', 'bp-scholar' );
			} else {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="membership-requested" rel="membership-requested" title="' . __( 'Membership Requested', 'bp-scholar' ) . '" href="' . bp_get_group_permalink( $group ) . '">' . __( 'Membership Requested', 'bp-scholar' ) . '</a>';
			}
		}
		...
		if ( !groups_leave_group( $group->id ) ) {
			_e( 'Error leaving group', 'bp-scholar' );
		} else {
			if ( 'public' == $group->status ) {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="join-group" rel="join" title="' . __( 'Join Group', 'bp-scholar' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'join', 'groups_join_group' ) . '">' . __( 'Join Group', 'bp-scholar' ) . '</a>';
			} else if ( 'private' == $group->status ) {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="request-membership" rel="join" title="' . __( 'Request Membership', 'bp-scholar' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'request-membership', 'groups_send_membership_request' ) . '">' . __( 'Request Membership', 'bp-scholar' ) . '</a>';
			}
		}
	}
}
...
function bp_dtheme_ajax_close_notice() {
	...
	if ( !isset( $_POST['notice_id'] ) ) {
		echo "-1<div id='message' class='error'><p>" . __('There was a problem closing the notice.', 'bp-scholar') . '</p></div>';
	} else {
...
function bp_dtheme_ajax_messages_send_reply() {
	...
	if ( $result ) { ?>
		<div class="message-box new-message">
			<div class="message-metadata">
				...
				<strong><a href="<?php echo $bp->loggedin_user->domain ?>"><?php echo $bp->loggedin_user->fullname ?></a> <span class="activity"><?php printf( __( 'Sent %s', 'bp-scholar' ), bp_core_time_since( bp_core_current_time() ) ) ?></span></strong>
	...
	<?php
		...
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem sending that reply. Please try again.', 'bp-scholar' ) . '</p></div>';
	}
}
...
function bp_dtheme_ajax_message_markunread() {
	...
	if ( !isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __('There was a problem marking messages as unread.', 'bp-scholar' ) . '</p></div>';
	} else {
...
function bp_dtheme_ajax_message_markread() {
	...
	if ( !isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __('There was a problem marking messages as read.', 'bp-scholar' ) . '</p></div>';
	} else {
		$thread_ids = explode( ',', $_POST['thread_ids'] );
...
function bp_dtheme_ajax_messages_delete() {
	...
	if ( !isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem deleting messages.', 'bp-scholar' ) . '</p></div>';
	} else {
		$thread_ids = explode( ',', $_POST['thread_ids'] );
		...
		_e( 'Messages deleted.', 'bp-scholar' );
	}
}
